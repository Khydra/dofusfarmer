	...
	render = () => {
  	...
  	drawRecipes = () => {
  		...
  		Object.keys(this.recipesShown).forEach((key, i) => {
  			...
  			let exp = expJobData[campesinoRecipeData[key].item.level + 1];

  			if (campesinoRecipeData[key].item.level > this.component.component.player.jobs['campesino'].level) {
  				...
  				exp = Math.floor(expJobData[campesinoRecipeData[key].item.level + 1] * (1 - (0.05 * (this.component.component.player.jobs['campesino'].level - campesinoRecipeData[key].item.level))));
  				if (exp <= 0) exp = 1;
  			}
  			
  			this.recipeSlotImage[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-image', image: campesinoRecipeData[key].item.image }).element; 
			this.recipeSlotName[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-name', text: campesinoRecipeData[key].item.name}).element; 
			this.recipeSlotLevel[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-level', text: `Nivel ${campesinoRecipeData[key].item.level}`}).element; 
			...
			this.recipeSlotIngredientContainer[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-ingredient-container'}).element; 
  			...
  			this.tooltip = new Tooltip(this.recipeSlotImage[i], campesinoRecipeData[key].item, 'jobWindow', this);
		    // Crear una función manejadora para este slot
		    const clickHandler = () => {
		     	const recipe = campesinoRecipeData[Object.keys(this.recipesShown)[i]];
		     	if (campesinoRecipeData[key].item.level > this.component.component.player.jobs['campesino'].level) return;
