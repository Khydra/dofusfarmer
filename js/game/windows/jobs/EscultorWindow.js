...
import { escultorRecipeData } from '../../data/item/recipeData.js';
import { resourceData } from '../../data/item/resourceData.js';
import { expJobData } from '../../data/class/expData.js';

export class EscultorWindow extends Window { 
	constructor(component) {
		const title = "Escultor";
	    const width = 850;
	    const height = 485;
	    const x = 530; 
	    const y = 80; 
	...
	render = () => {
  	...
  	drawRecipes = () => {
  		...
  		Object.keys(this.recipesShown).forEach((key, i) => {
  			...
  			let exp = expJobData[escultorRecipeData[key].item.level + 1];

  			if (escultorRecipeData[key].item.level > this.component.component.player.jobs['escultor'].level) {
  				...
  				exp = Math.floor(expJobData[escultorRecipeData[key].item.level + 1] * (1 - (0.05 * (this.component.component.player.jobs['escultor'].level - escultorRecipeData[key].item.level))));
  				if (exp <= 0) exp = 1;
  			}
  			
  			this.recipeSlotImage[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-image', image: escultorRecipeData[key].item.image }).element; 
			this.recipeSlotName[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-name', text: escultorRecipeData[key].item.name}).element; 
			this.recipeSlotLevel[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-level', text: `Nivel ${escultorRecipeData[key].item.level}`}).element; 
			...
			this.recipeSlotIngredientContainer[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-ingredient-container'}).element; 
  			this.drawIngredients(escultorRecipeData[key].recipe, this.recipeSlotIngredientContainer[i]);


  			this.tooltip = new Tooltip(this.recipeSlotImage[i], escultorRecipeData[key].item, 'jobWindow', this);
		    // Crear una función manejadora para este slot
		    const clickHandler = () => {
		     	const recipe = escultorRecipeData[Object.keys(this.recipesShown)[i]];
		     	if (escultorRecipeData[key].item.level > this.component.component.player.jobs['escultor'].level) return;
		      	if (this.component.craftWindow.isOpen) {
		        	this.component.craftWindow.update(recipe, 'escultor');
		      	} else {
		        	this.component.craftWindow.open(recipe, 'escultor');
		      	}
		    };
  	...
  	filterRecipes = () => {
  		let recipes = this.filterByLevel(escultorRecipeData, this.filter.level[0], this.filter.level[1]);
