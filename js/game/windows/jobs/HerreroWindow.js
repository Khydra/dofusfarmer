...
import { herreroRecipeData } from '../../data/item/recipeData.js';
import { resourceData } from '../../data/item/resourceData.js';
import { expJobData } from '../../data/class/expData.js';

export class HerreroWindow extends Window { 
	...
	render = () => {
    	...
    	new Selector(this.settingsContainer, "Mostrar", ["Todo", "Arma"], 'job-recipe-type', (value) => {
    		let sortValues =  ["todo", "arma"]
		    ...
		    const clickHandler = () => {
		     	...
		     	if (herreroRecipeData[key].level > this.component.component.player.jobs['herrero'].level) return;
		      	if (this.component.craftWindow.isOpen) {
		        	...
		        	this.component.craftWindow.open(recipe, 'herrero');
		      	}
		    };
  	...
  	filterRecipes = () => {
  		let recipes = this.filterByLevel(herreroRecipeData, this.filter.level[0], this.filter.level[1]);
