...
import { lenadorRecipeData } from '../../data/item/recipeData.js';
import { resourceData } from '../../data/item/resourceData.js';
import { expJobData } from '../../data/class/expData.js';

export class LenadorWindow extends Window { 
	...
	render = () => {
    	...
    	new Selector(this.settingsContainer, "Mostrar", ["Todo", "Tabla", "Concentrado"], 'job-recipe-type', (value) => {
    		let sortValues =  ["todo", "tabla", "concentrado"]
  	...
  	drawRecipes = () => {
  		...
  		Object.keys(this.recipesShown).forEach((key, i) => {
  			...
  			let exp = expJobData[lenadorRecipeData[key].level + 1];

  			if (lenadorRecipeData[key].level > this.component.component.player.jobs['lenador'].level) {
  				...
  				exp = Math.floor(expJobData[lenadorRecipeData[key].level + 1] * (1 - (0.05 * (this.component.component.player.jobs['lenador'].level - lenadorRecipeData[key].level))));
  			}
  			exp = Math.ceil(exp / 15);
  			if (exp <= 0) exp = 1;
  			
  			this.recipeSlotImage[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-image', image: lenadorRecipeData[key].image }).element; 
			this.recipeSlotName[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-name', text: lenadorRecipeData[key].name}).element; 
			this.recipeSlotLevel[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-level', text: `Nivel ${lenadorRecipeData[key].level}`}).element; 
			...
			this.recipeSlotIngredientContainer[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-ingredient-container'}).element; 
  			this.drawIngredients(lenadorRecipeData[key].recipe, this.recipeSlotIngredientContainer[i]);


  			this.tooltip = new Tooltip(this.recipeSlotImage[i], lenadorRecipeData[key], 'jobWindow', this);
		    // Crear una función manejadora para este slot
		    const clickHandler = () => {
		     	const recipe = lenadorRecipeData[Object.keys(this.recipesShown)[i]];
		     	if (lenadorRecipeData[key].level > this.component.component.player.jobs['lenador'].level) return;
		      	if (this.component.craftWindow.isOpen) {
		        	this.component.craftWindow.update(recipe, 'lenador');
		      	} else {
		        	this.component.craftWindow.open(recipe, 'lenador');
		      	}
		    };
  	...
  	filterRecipes = () => {
  		let recipes = this.filterByLevel(lenadorRecipeData, this.filter.level[0], this.filter.level[1]);
