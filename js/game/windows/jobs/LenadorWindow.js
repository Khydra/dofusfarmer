	...
	render = () => {
  	...
  	drawRecipes = () => {
  		...
  		Object.keys(this.recipesShown).forEach((key, i) => {
  			...
  			let exp = expJobData[lenadorRecipeData[key].item.level + 1];

  			if (lenadorRecipeData[key].item.level > this.component.component.player.jobs['lenador'].level) {
  				...
  				exp = Math.floor(expJobData[lenadorRecipeData[key].item.level + 1] * (1 - (0.05 * (this.component.component.player.jobs['lenador'].level - lenadorRecipeData[key].item.level))));
  				if (exp <= 0) exp = 1;
  			}
  			
  			this.recipeSlotImage[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-image', image: lenadorRecipeData[key].item.image }).element; 
			this.recipeSlotName[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-name', text: lenadorRecipeData[key].item.name}).element; 
			this.recipeSlotLevel[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-level', text: `Nivel ${lenadorRecipeData[key].item.level}`}).element; 
			...
			this.recipeSlotIngredientContainer[i] = new Element(this.recipeSlot[i], { className: 'job-recipe-slot-ingredient-container'}).element; 
  			...
  			this.tooltip = new Tooltip(this.recipeSlotImage[i], lenadorRecipeData[key].item, 'jobWindow', this);
		    // Crear una función manejadora para este slot
		    const clickHandler = () => {
		     	const recipe = lenadorRecipeData[Object.keys(this.recipesShown)[i]];
		     	if (lenadorRecipeData[key].item.level > this.component.component.player.jobs['lenador'].level) return;
