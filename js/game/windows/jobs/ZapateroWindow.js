...
import { zapateroRecipeData } from '../../data/item/recipeData.js';
import { resourceData } from '../../data/item/resourceData.js';
import { expJobData } from '../../data/class/expData.js';

export class ZapateroWindow extends Window { 
	constructor(component) {
		const title = "Zapatero";
	    const width = 850;
	    const height = 485;
	    const x = 530; 
	    const y = 80; 
	...
	render = () => {
    	...
    	new Selector(this.settingsContainer, "Mostrar", ["Todo", "Botas", "Cinturón"], 'job-recipe-type', (value) => {
    		let sortValues =  ["todo", "botas", "cinturon"]
		    ...
		    const clickHandler = () => {
		     	...
		     	if (zapateroRecipeData[key].item.level > this.component.component.player.jobs['zapatero'].level) return;
		      	if (this.component.craftWindow.isOpen) {
		        	...
		        	this.component.craftWindow.open(recipe, 'zapatero');
		      	}
		    };
  	...
  	filterRecipes = () => {
  		let recipes = this.filterByLevel(zapateroRecipeData, this.filter.level[0], this.filter.level[1]);
